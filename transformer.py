...
class DecoderOnlyAFT(nn.Module):
    ...
    def compute_loss(self, x_in: torch.Tensor, x_out: torch.Tensor, seg_len=None):
        seg_len = seg_len or self.sequence_len

        n_segs = (self.sequence_len + seg_len - 1) // seg_len # calculate the number of segments needed to cover the entire sequence

        seq_loss = 0.0
        decoder_out = self.forward(x_in, training=True)

        for n_seg in range(n_segs):
            seg_start, seg_end = n_seg * seg_len, min((n_seg + 1) * seg_len, self.sequence_len)
